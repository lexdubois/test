...
//            if (traceList.get(i).getX() < 0 || traceList.get(i).getY() < 0)
    ...
    public void addCoords(Coords coords){
        if ((traceList.size() >= 2) && ((coords.getX() - (getCoordsByIndex(traceList.size() - 1).getX())) == ((getCoordsByIndex(traceList.size() - 1).getX()) - (getCoordsByIndex(traceList.size() - 2).getX())))
            && (((coords.getX() > 0 && traceList.getLast().getX() > 0) || (coords.getX() < 0 &&  traceList.getLast().getX() < 0))
            || (((coords.getY() > 0 && traceList.getLast().getY() > 0) ||((coords.getY() < 0 &&  traceList.getLast().getY() < 0)))))){
            head = coords;
        }
        else {
    ...
    public void maxLength() {
        ...
        double surplus = getTraceLength() - max;
            ...
            while (surplus > 0 && segment < surplus) {
                ...
                surplus -= segment;
                segment = (getCoordsByIndex(traceList.size() - 1).distanceTo(getCoordsByIndex(traceList.size() - 2)));
            }
